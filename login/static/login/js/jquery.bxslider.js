...
;(function($j){
	...
	$j.fn.bxSlider = function(options){
		...
		var start = function(){
			...
			if (slider.settings.responsive) $j(window).bind('resize', resizeWindow);
		...
		var appendCaptions = function(){
			...
			slider.children.each(function(index){
				...
				if (title != undefined && ('' + title).length) {
                    $j(this).append('<div class="bx-caption"><span>' + title + '</span></div>');
                }
			});
		}
		...
		el.goToSlide = function(slideIndex, direction){
			...
			if(slider.settings.mode == 'fade'){
				...
				slider.children.eq(slider.active.index).css('zIndex', slider.settings.slideZIndex+1).fadeIn(slider.settings.speed, function(){
					$j(this).css('zIndex', slider.settings.slideZIndex);
					updateAfterSlideTransition();
				});
		...
		el.destroySlider = function(){
			...
			slider.children.each(function() {
				$j(this).data("origStyle") != undefined ? $j(this).attr("style", $j(this).data("origStyle")) : $j(this).removeAttr('style');
			});
			$j(this).data("origStyle") != undefined ? this.attr("style", $j(this).data("origStyle")) : $j(this).removeAttr('style');
			$j(this).unwrap().unwrap();
			...
			clearInterval(slider.interval);
			if(slider.settings.responsive) $j(window).unbind('resize', resizeWindow);
		}
